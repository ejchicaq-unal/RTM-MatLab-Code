function [model, BckwdSnapshot] = BackModelingFunction(V,data,nz,nx,dx,nt,dt,M,am,N)
...
% rw(nt)            Ricker wavelet
% v(nz,nx)          velocity model
% nx                number of horizontal samples
% nz                number of depth samples
% nt                numer of time samples
% dx                horizontal distance per sample in meters (horizontal grid interval)
% dz                depth distance per sample in meters (vertical grid interval)
% dt                time difference per sample in miliseconds
% ixs               x shot position
% izs               z shot position
...
fdm  = zeros(nzz,nxx,3);    % this matrix stores the values of the wave field
fdmII  = zeros(nzz,nxx,3);  % this matrix stores the values of the wave field y zone II
BckwdSnapshot = zeros(nzz,nxx,nt);   % 'snapshot' matrix stores the values of the wave field para
                                     % for each time increment

%% Wave modeling in time direction

fdm(2*M+N+1,2*M+N+1:end-(2*M+N),1) = data(:,nt);
fdm(2*M+N+1,2*M+N+1:end-(2*M+N),2) = data(:,nt-1);
fdm(2*M+N+1,2*M+N+1:end-(2*M+N),3) = data(:,nt-2);
...
for it = (nt-1):-1:1 % time iteration cycle
    ...
    for i=1:M % index in the 'x' direction
        for j=1:M % index in the 'z' direction
   ...
   % Bottom contour
    ...
    fdmII(:,:,1) = fdmII(:,:,2);
    fdmII(:,:,2) = fdmII(:,:,3);
    fdm(2*M+N+1,2*M+N+1:end-(2*M+N),2) = data(:,it); % insert surface boundary wavefield

    % update data

    BckwdSnapshot(:,:,it) = fdm(:,:,2);
    model = fdm(:,:,1);
end
